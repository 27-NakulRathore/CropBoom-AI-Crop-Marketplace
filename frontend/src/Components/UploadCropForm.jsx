...
const GEMINI_API_KEY = 'AIzaSyAmCri7ZlH3JTm9VJYN4Y79JC-eaCYgx_Y'; 
...
function UploadCropFormSimplified() {
  ...
  const [analysisData, setAnalysisData] = useState({}); // For storing parsed analysis data
  const [loading, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [isDragging, setIsDragging] = useState(false);
  const location = useLocation();
  const email = location.state?.email;
  console.log("email resived at upload page ",email);
  ...
  const parseAnalysisResult = (result) => {
    ...
    lines.forEach(line => {
      if (line.startsWith('Quality:')) {
        ...
        // Extract just the numbers if needed
  ...
  const handleAnalyze = async (e) => {
    ...
    try {
      ...
      if (analysis.error) {
        toast.error('Analysis failed: ' + analysis.error);
      } else {
  ...
  const handleUpload = async (e) => {
   ...
   console.log("data uplooded",formData);
